...
(
   ...
   for( i =0; i<nx; i++){
      for(j=0; j<ny; j++){
         dis = dz[i+1][j+1];
         /* first gray color channel */
         jg11 = Jg_11[0*nx*ny + i*ny + j];
         jg12 = Jg_12[0*nx*ny + i*ny + j];
         jg22 = Jg_22[0*nx*ny + i*ny + j];
         wgr = jg11*dis*dis + 2*dis*jg12 + jg22;
         wgr = 1.0 / sqrt(wgr + epsi);

         /* second gray color channel */
         jg11 = Jg_11[1*nx*ny + i*ny + j];
         jg12 = Jg_12[1*nx*ny + i*ny + j];
         jg22 = Jg_22[1*nx*ny + i*ny + j];
         wgg = jg11*dis*dis + 2*dis*jg12 + jg22;
         wgg = 1.0 / sqrt(wgg + epsi);

         /* third gray color channel */
         jg11 = Jg_11[2*nx*ny + i*ny + j];
         jg12 = Jg_12[2*nx*ny + i*ny + j];
         jg22 = Jg_22[2*nx*ny + i*ny + j];
         wgb = jg11*dis*dis + 2*dis*jg12 + jg22;
         wgb = 1.0 / sqrt(wgb + epsi);

         /* first gradient color channel */
         jG11 = JG_11[0*nx*ny + i*ny + j];
         jG12 = JG_12[0*nx*ny + i*ny + j];
         jG22 = JG_22[0*nx*ny + i*ny + j];
         wGr = jG11*dis*dis + 2*dis*jG12 + jG22;
         wGr = gamma* 1.0 / sqrt(wGr + epsi);

         /* seconde gradient color channel */
         jG11 = JG_11[1*nx*ny + i*ny + j];
         jG12 = JG_12[1*nx*ny + i*ny + j];
         jG22 = JG_22[1*nx*ny + i*ny + j];
         wGg = jG11*dis*dis + 2*dis*jG12 + jG22;
         wGg = gamma* 1.0 / sqrt(wGg + epsi);

         /* third gradient color channel */
         jG11 = JG_11[2*nx*ny + i*ny + j];
         jG12 = JG_12[2*nx*ny + i*ny + j];
         jG22 = JG_22[2*nx*ny + i*ny + j];
         wGb = jG11*dis*dis + 2*dis*jG12 + jG22;
         wGb = gamma* 1.0 / sqrt(wGb + epsi);

         P[i+1][j+1] =  wgr*Jg_11[0*nx*ny + i*ny + j]
                        + wgg*Jg_11[1*nx*ny + i*ny + j]
                        + wgb*Jg_11[2*nx*ny + i*ny + j]
                        + wGr*JG_11[0*nx*ny + i*ny + j]
                        + wGg*JG_11[1*nx*ny + i*ny + j]
                        + wGb*JG_11[2*nx*ny + i*ny + j];

         Q[i+1][j+1] =  wgr*Jg_12[0*nx*ny + i*ny + j]
                        + wgg*Jg_12[1*nx*ny + i*ny + j]
                        + wgb*Jg_12[2*nx*ny + i*ny + j]
                        + wGr*JG_12[0*nx*ny + i*ny + j]
                        + wGg*JG_12[1*nx*ny + i*ny + j]
                        + wGb*JG_12[2*nx*ny + i*ny + j];
